...
var isCSSProperty = (key, value) =>
  !includes("@:&", key[0]) &&
  (includes("rg", (typeof value)[5]) || Array.isArray(value));
    ...
    join(
      screen.map((screen2) => {
        ...
        return (
          screen2.raw ||
          join(
            Object.keys(screen2).map(
              (feature) => `(${feature}-width:${screen2[feature]})`
            ),
            " and "
          )
        );
...
var parseString = (token, isVariant) => {
  let buffer = "";
  for (
    let char, dynamic = false, position2 = 0;
    (char = token[position2++]);
    ...
    switch (char) {
      case ":":
        buffer =
          buffer &&
          startGrouping(
            ":" + (token[position2] == char ? token[position2++] : "") + buffer
          );
        break;
      case "(":
...
var parse = (tokens) => {
  ...
  if (Array.isArray(tokens[0]) && Array.isArray(tokens[0].raw)) {
    buildStatics(tokens[0]).forEach((apply2, index) =>
      apply2(tokens[index + 1])
    );
  } else {
    parseToken(tokens);
  }
...
var stringifyRule = (rule, directive2 = rule.d) =>
...
var alias = (handler, name) => (params, context) =>
...
var inset = (params, {theme: theme2}, id) =>
  (_ = theme2("inset", params)) && {[id]: _};
var opacityProperty = (params, theme2, id, section = id) =>
  (_ = theme2(section + "Opacity", tail(params))) && {
    [`--tw-${id}-opacity`]: _,
  };
var parseColorComponent = (chars, factor) =>
...
var withOpacityFallback = (property2, kind, color) =>
  color && typeof color == "string"
    ? (_ = asRGBA(color, kind + "-opacity")) && _ !== color
      ...
      : {[property2]: color}
    : void 0;
var transparentTo = (color) =>
  ($ = asRGBA(color, "", "0")) == _ ? "transparent" : $;
...
var placeHelper = (property2, params) =>
  params[0] && {
    [property2]:
      (includes("wun", (params[0] || "")[3]) ? "space-" : "") + params[0],
  };
...
var borderEdges = (params, context, id) => {
  var _a;
  const edges2 =
    (_a = expandEdges(params[0])) == null ? void 0 : _a.map(capitalize);
  if (edges2) {
    params = tail(params);
  }
...
var corePlugins = {
  ...
  flex(params, context, id) {
    switch (params[0]) {
      ...
      case "col":
        return {
          flexDirection: join(
            params[0] == "col" ? ["column", ...tail(params)] : params
          ),
        };
      case "nowrap":
      case "wrap":
  ...
  items: (params) =>
    params[0] && {
      alignItems: includes(["start", "end"], params[0])
        ? "flex-" + params[0]
        : join(params),
    },
  ...
  place: (params) =>
    params[0] && placeHelper("place-" + params[0], tail(params)),
  overscroll: (params) =>
    params[0] && {
      ["overscrollBehavior" + (params[1] ? "-" + params[0] : "")]:
        params[1] || params[0],
    },
  ...
  bg(params, {theme: theme2}, id) {
    switch (params[0]) {
      ...
      case "no":
        return (
          params[1] == "repeat" && propertyValue("backgroundRepeat")(params)
        );
      case "repeat":
    ...
    (_ =
      params[0] == "opacity"
        ...
        : withOpacityFallback(
            "color",
            "placeholder",
            theme2("placeholderColor", params)
          )) && {
      "&::placeholder": _,
    },
  ...
  rounded: (params, context, id) =>
    corners(
      context.theme("borderRadius", tail(params), ""),
      params[0],
      "border",
      "radius"
  ...
  container: (params, {theme: theme2}) => {
    ...
    const paddingFor = (screen) =>
      (_ =
        padding2 &&
        (typeof padding2 == "string"
          ? padding2
          : padding2[screen] || padding2.DEFAULT))
        ? {
            paddingRight: _,
            paddingLeft: _,
          }
...
var mode = (report) => ({
  unknown(section, key = [], optional, context) {
    if (!optional) {
      this.report(
        {id: "UNKNOWN_THEME_VALUE", key: section + "." + join(key)},
        context
      );
    }
  },
...
var makeThemeResolver = (config) => {
  ...
  const deref = (theme3, section) => {
    ...
    const value =
      typeof base == "function" ? base(resolve, resolveContext) : base;
...
var prepareVariantSelector = (variant) =>
    ...
    return {
      ...
      "&" +
        variant.replace(
          NOT_PREFIX_RE,
          (_4, not, variant2) =>
            not + "(" + prepareVariantSelector(":" + variant2) + ")"
        )]: translation,
    };
  };
...
var pseudoPrecedence = (pseudoClass) =>
  ...
  (~(_3 = PRECEDENCES_BY_PSEUDO_CLASS.indexOf(
    ...
    ? _3
    : 17);
var makeVariantPresedenceCalculator =
  (theme2, variants) => (presedence, variant) =>
    presedence |
...
var declarationPropertyPrecedence = (property2) =>
  property2[0] == "-"
    ...
    : seperatorPrecedence(property2) +
      ((_3 =
        /^(?:(border-(?!w|c|sty)|[tlbr].{2,4}m?$|c.{7}$)|([fl].{5}l|g.{8}$|pl))/.exec(
          property2
        ))
        ? +!!_3[1] || -!!_3[2]
        : 0) +
...
var serialize = (prefix, variants, context) => {
  ...
  const stringifyDeclaration = (property2, value, important) => {
    ...
    return Array.isArray(value)
      ? join(
          value
            .filter(Boolean)
            .map((value2) => prefix(property2, tagVars(value2), important)),
          ";"
        )
  ...
  const stringify2 = (atRules, selector, presedence, css, important) => {
    if (Array.isArray(css)) {
      css.forEach(
        (css2) =>
          css2 && stringify2(atRules, selector, presedence, css2, important)
      );
      return;
    }
    let declarations = "";
      ...
      if (isCSSProperty(key, value)) {
        ...
        if (key[0] == "@") {
          if (key[1] == "g") {
            ...
            stringify2([], "", 0, value, important);
            const waypoints = rules2.splice(
              currentSize,
              rules2.length - currentSize
            );
            rules2.push({
              r: stringifyBlock(
                join(
            ...
            if (key[2] == "c") {
              key = buildMediaQuery(
                context.theme("screens", tail(key, 8).trim())
              );
            }
            stringify2(
              [...atRules, key],
    ...
    if (numberOfDeclarations) {
      rules2.push({
        r: atRules.reduceRight(
          stringifyBlock,
          stringifyBlock(declarations, selector)
        ),
        p:
          presedence * (1 << 8) +
...
var loadMode = (mode2) =>
  (typeof mode2 == "string"
    ? {t: strict, a: warn, i: silent}[mode2[1]]
    : mode2) || warn;
...
var configure = (config = {}) => {
  ...
  const context = {
    ...
    css: (rules2) => {
      ...
      try {
        ...
        for (
          let index = lastTranslationsIndex;
          index < lastTranslations.length;
          index++
        ) {
  ...
  const sheet =
    config.sheet ||
    (typeof window == "undefined" ? voidSheet() : cssomSheet(config));
    ...
    if (className == null) {
      ...
      if (translation && typeof translation == "object") {
        ...
        if (translateDepth) {
          ...
          const layer =
            typeof rule.d == "function"
              ? typeof translation._ == "string"
                ? 1
                : 3
              : 2;
          className =
            hash || typeof rule.d == "function"
              ? (hash || cyrb32)(layer + rule.$)
              : rule.$;
  ...
  const process = (tokens) =>
    join(parse(tokens).map(convert).filter(Boolean), " ");
